...
-- Secure alerts table - replace system policy with community head policy
DROP POLICY IF EXISTS "System can create alerts" ON public.alerts;
DROP POLICY IF EXISTS "Community heads can create alerts" ON public.alerts;

CREATE POLICY "Only community heads can create alerts" 
ON public.alerts 
FOR INSERT 
WITH CHECK (
  community_id IN (
...
-- Update sensor data policy for better security
DROP POLICY IF EXISTS "Allow ESP32 to insert sensor data" ON public.sensor_data;

CREATE POLICY "Secure sensor data insertion" 
